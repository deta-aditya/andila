...
body.on('click', '#clone-email', function () {
	var subagentEmail = $('[name="email"]');
	var userEmail = $('[name="user_email"]');

	userEmail.val(subagentEmail.val());
});

// Handle create subagent form submition
body.on('submit', '#form-subagent-create', function () {
	...
	var data = {
		...
		contract_value: andila.form.get(form, 'contract_value'),
		address: {},
		user: {
	...
	andila.api.post(andila.apiLocation + '/subagents', data).done(function (res) {
		andila.html.btnSuccess(submit);
		andila.helper.redirect(andila.appLocation + '/subagents', {post: res.model.id});
	}).fail([function () {
