 ...
 * Global variables
 */
var addressBox = $('#andila-subagent-address');
...
// Handle edit subagent form submition
body.on('submit', '#form-subagent-edit', function () {
	...
	if (andila.form.has(form, 'owner')) {
		data.owner = andila.form.get(form, 'owner');
	}

	if (andila.form.has(form, 'contract_value')) {
		data.contract_value = andila.form.get(form, 'contract_value');
	}
	...
	andila.api.put(andila.apiLocation + '/subagents/' + andila.form.get(form, 'id'), data).done(function (res) {
		andila.html.btnSuccess(submit);
		andila.helper.redirect(andila.appLocation + '/subagents', {put: res.model.id});
	}).fail([function () {
